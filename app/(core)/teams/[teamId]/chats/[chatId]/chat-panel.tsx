...
import { ChatResponseI, CodeVersionMappingSchemaI } from "@/utils/types";
...
const ChatPanel: React.FC<ChatPanelProps> = ({ version, teamId }) => {
  ...
  const { data: chats, isLoading } = useQuery({
    queryKey: ["chats", teamId, query],
    queryFn: () => chatActions.getChats(teamId, query),
  });

  const createChatMutation = useMutation({
    mutationFn: () => chatActions.initiateChat(teamId, versionId),
    onSuccess: (data) => {
      setSelectedChatId(data.id);
      queryClient.invalidateQueries({ queryKey: ["chats", teamId] });
    },
  });
