...
import { MemberInviteSchema, MemberRoleEnum, MemberSchema } from "@/utils/types";
...
const InvitesList: React.FC<InvitesListProps> = ({ curMember, invites, isLoading }) => {
  ...
  const updateInviteMutation = useMutation({
    mutationFn: async (data: { inviteId: string; role: MemberRoleEnum }) =>
      teamActions.updateInvite(data.inviteId, { role: data.role }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["invites"] });
    },
  });
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (selectedInvite) {
                  console.log(selectedInvite.id, selectedRole);
              ...
              invites.map((invite) => (
                <TableRow key={invite.id}>
                  ...
                  <TableCell
                    ...
                    aria-disabled={curMember.role !== MemberRoleEnum.OWNER}
                  >
                    <DropdownMenu>
                      <DropdownMenuTrigger
                        asChild
                        disabled={curMember.role !== MemberRoleEnum.OWNER}
                      >
