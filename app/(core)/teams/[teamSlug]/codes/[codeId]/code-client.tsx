...
import { CodeSourceSchemaI, CodeVersionMappingSchemaI, UserDetailedSchemaI } from "@/utils/types";
...
const CodeClient: React.FC<{
  ...
  user: UserDetailedSchemaI | null;
}> = ({ teamSlug, sources, version, user }) => {
      ...
      <div className="grid pb-4 lg:pt-4 pr-2" style={{ gridTemplateColumns: "250px 1fr" }}>
        ...
        <div className="flex justify-between gap-10">
          ...
          <div className="flex items-center justify-end w-full gap-4 text-sm text-muted-foreground">
            ...
            {!!user && (
              <CreateAnalysisVersionButton version={version} teamSlug={teamSlug} user={user} />
            )}
          </div>
        </div>
      </div>
      <SourcesViewer sources={sources} teamSlug={teamSlug} codeId={version.id} />
    </ScrollArea>
  );
};
