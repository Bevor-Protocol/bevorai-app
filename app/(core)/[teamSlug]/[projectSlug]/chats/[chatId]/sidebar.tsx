    ...
    <aside className="w-72 shrink-0 flex flex-col bg-background gap-6 relative">
            ...
            <div className="flex flex-col gap-2">
              {chatsQuery.data?.results.map((chat) => (
                <Button
                  ...
                  asChild
                >
                  <Link href={`/${teamSlug}/${projectSlug}/chats/${chat.id}`}>
      ...
      {chatQuery.data && (
        <div>
          ...
          <div className="space-y-2">
            <Link
              href={`/${teamSlug}/${projectSlug}/codes/${chatQuery.data.code_mapping.id}`}
            ...
            {chatQuery.data.analysis_node && (
              <Link
                href={`/${teamSlug}/${projectSlug}/analysis-threads/${chatQuery.data.analysis_thread_id}/nodes/${chatQuery.data.analysis_node_id}`}
                className="block hover:opacity-80 transition-opacity"
              >
