...
const CodeVersionMenu: React.FC<{
  ...
  const uploadNewerPath = `/${teamSlug}/${projectSlug}/codes/new?parentId=${version.id}`;
  const baseNewAnalysisPath = `/${teamSlug}/${projectSlug}/analyses/new?codeVersionId=${version.id}`;
  const viewAllAnalysesPath = `/${teamSlug}/${projectSlug}/analyses?code_version_id=${version.id}&user=unset`;
  const viewUserAnalysesPath = `/${teamSlug}/${projectSlug}/analyses?code_version_id=${version.id}`;
  ...
  const handleUseParent = (): void => {
    ...
    router.push(
      `/${teamSlug}/${projectSlug}/analyses/new?codeVersionId=${version.id}&parentVersionId=${parentId}`,
    );
    setOpen(false);
  };
  ...
  const handleConfirmParent = (): void => {
    if (selectedParentId) {
      router.push(
        `/${teamSlug}/${projectSlug}/analyses/new?codeVersionId=${version.id}&parentVersionId=${selectedParentId}`,
      );
    } else {
      router.push(baseNewAnalysisPath);
    }
