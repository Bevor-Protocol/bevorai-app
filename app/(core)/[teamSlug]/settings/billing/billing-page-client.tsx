  ...
  const checkoutMutation = useMutation({
    mutationFn: () =>
      billingActions.updatePaymentMethod(teamSlug, {
        success_url: `${window.location.origin}/${teamSlug}/settings/billing?success=true`,
        cancel_url: `${window.location.origin}/${teamSlug}/settings/billing?canceled=true`,
      }),
    onSuccess: (data) => {
    ...
    <>
      <div className="border border-border rounded-lg p-6 mb-8">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border">
          {isTrial ? (
            <Link href={`/${teamSlug}/settings/plans`}>
